...
def evaluate(model, data_loader, device):
    ...
    confusion_metric = utils.ConfusionMatrixMetric(num_classes=2, threshold=0.5)
    ...
    with torch.no_grad():
        for images, targets in metric_logger.log_every(data_loader, 100, header):
            ...
            confusion_metric.update(output, targets)

        mae_metric.gather_from_all_processes()
        f1_metric.reduce_from_all_processes()
        confusion_metric.gather_from_all_processes()

    return mae_metric, f1_metric, confusion_metric
